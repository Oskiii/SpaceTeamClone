...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] CommandNetworkObject = null;
		public GameObject[] CommandsNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is CommandNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CommandNetworkObject.Length > 0 && CommandNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CommandNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
			...
			else if (obj is CommandsNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CommandsNetworkObject.Length > 0 && CommandsNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CommandsNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateCommand instead, its shorter and easier to type out ;)")]
		public CommandBehavior InstantiateCommandNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CommandNetworkObject[index]);
			var netBehavior = go.GetComponent<CommandBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CommandBehavior>().networkObject = (CommandNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateCommands instead, its shorter and easier to type out ;)")]
		public CommandsBehavior InstantiateCommandsNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CommandsNetworkObject[index]);
			var netBehavior = go.GetComponent<CommandsBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CommandsBehavior>().networkObject = (CommandsNetworkObject)obj;
		...
		public CommandBehavior InstantiateCommand(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CommandNetworkObject[index]);
			var netBehavior = go.GetComponent<CommandBehavior>();
		...
		public CommandsBehavior InstantiateCommands(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CommandsNetworkObject[index]);
			var netBehavior = go.GetComponent<CommandsBehavior>();
